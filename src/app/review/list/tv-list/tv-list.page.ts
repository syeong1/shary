...
export class TvListPage implements OnInit {
  ...
  constructor(private tvService: TvService, private activatedRoute: ActivatedRoute, private router: Router, private route: ActivatedRoute, private reviewService: ReviewService, private actionSheetController: ActionSheetController) {
  ...
  getReviewList() {
    this.reviewService.getReviewList('tv', this.reviewbookId).subscribe((data: any[]) => {
      if (this.filter === '등록일순') {
  ...
  openReivewDetailPage(reviewId) {
    this.router.navigate(['tv/detail', reviewId]);
  }
